package internal

import (
	"os"
	"os/exec"
	"path/filepath"
	"testing"
	"time"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

// minimalJPEG is a 1×1 black JPEG used as a test fixture.
var minimalJPEG = []byte{
	,
}

// requireExiftool skips the test if the exiftool binary is not in PATH.
func requireExiftool(t *testing.T) {
	t.Helper()
	if _, err := exec.LookPath("exiftool"); err != nil {
		t.Skip("exiftool not found in PATH — skipping integration test")
	}
}

func TestExtractDate_DateTimeOriginal(t *testing.T) {
	requireExiftool(t)

	tmpDir := t.TempDir()
	imgPath := filepath.Join(tmpDir, "test.jpg")
	require.NoError(t, os.WriteFile(imgPath, minimalJPEG, 0644))

	const dateTimeOriginal = "2023:01:01 12:00:00"
	out, err := exec.Command("exiftool", "-DateTimeOriginal="+dateTimeOriginal, imgPath).CombinedOutput()
	require.NoError(t, err, "exiftool failed: %s", out)

	svc, err := NewExifToolService()
	require.NoError(t, err)
	defer svc.Close()

	got, tag, err := svc.ExtractDate(imgPath, false, false)
	require.NoError(t, err)

	expected, _ := ParseExifDate(dateTimeOriginal)
	assert.True(t, got.Equal(expected), "extracted date %v should equal %v", got, expected)
	assert.Equal(t, "DateTimeOriginal", tag)
}

func TestExtractDate_NoMetadata_ReturnsZeroTime(t *testing.T) {
	requireExiftool(t)

	tmpDir := t.TempDir()
	imgPath := filepath.Join(tmpDir, "bare.jpg")
	require.NoError(t, os.WriteFile(imgPath, minimalJPEG, 0644))

	svc, err := NewExifToolService()
	require.NoError(t, err)
	defer svc.Close()

	got, tag, err := svc.ExtractDate(imgPath, false, false)

	assert.NoError(t, err)
	assert.True(t, got.IsZero(), "expected zero time when no EXIF date is present")
	assert.Empty(t, tag)
}

func TestParseExifDate(t *testing.T) {
	cases := []struct {
		name    string
		input   string
		want    time.Time
		wantErr bool
	}{
		{
			name:  "with timezone",
			input: "2023:01:01 12:00:00-05:00",
			want:  time.Date(2023, 1, 1, 12, 0, 0, 0, time.FixedZone("", -5*60*60)),
		},
		{
			name:  "without timezone",
			input: "2023:01:01 12:00:00",
			want:  time.Date(2023, 1, 1, 12, 0, 0, 0, time.UTC),
		},
		{
			name:  "date only",
			input: "2023:01:01",
			want:  time.Date(2023, 1, 1, 0, 0, 0, 0, time.UTC),
		},
		{
			name:    "invalid format",
			input:   "2023-01-01 12:00:00",
			wantErr: true,
		},
		{
			name:    "empty string",
			input:   "",
			wantErr: true,
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := ParseExifDate(tc.input)
			if tc.wantErr {
				assert.Error(t, err)
				return
			}
			require.NoError(t, err)
			assert.True(t, got.Equal(tc.want), "got %v, want %v", got, tc.want)
		})
	}
}
